...
void main() {
	...
	vec3 darker = vec3(0.9,0.9,0.9); 

	//result.x = fTexcoord.x;
	// if (fTexcoord.x*fTexcoord.y < 0.05 || 	(1-fTexcoord.x*fTexcoord.y) < 0.05) {

	if (fTexcoord.x < 0.05 || fTexcoord.x > 0.95 ||
		fTexcoord.y < 0.05 || fTexcoord.y > 0.95) {
			result *= darker;
	}
